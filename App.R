...
# ── Colonne di default da mantenere (biologicamente rilevanti) ────────────────
DEFAULT_KEEP <- c(
...
TR <- list(
  it = list(
    ...
    privacy_body     = "Questa applicazione gira interamente nel tuo browser. Nessun dato viene inviato a server esterni: il file che carichi viene elaborato solo sul tuo dispositivo.",
  ...
  en = list(
    ...
    privacy_body     = "This application runs entirely in your browser. No data is sent to external servers: the file you upload is processed only on your device.",
...
parse_winrhizo <- function(filepath) {
  ...
  # Riga 1 = header principale; righe 2-5 = sub-header da scartare
  header <- strsplit(lines[1], "\t")[[1]]

  # Rinomina RHIZO 2021a → SampleId e rimuove colonna vuota finale
  header[header == "RHIZO 2021a"] <- "SampleId"
  header <- trimws(header)

  # Righe dati: dalla 6 in poi, skip sub-headers (righe 2-5)
  ...
  # Rimuove colonna vuota finale se presente
  df <- df[, colnames(df) != "", drop = FALSE]
  df
}

# ── Estrazione automatica trattamento e replica dal SampleId ─────────────────
# Trattamento: tutto ciò che precede il suffisso numerico finale (_1, _2, ...)
extract_treatment <- function(sample_ids) {
  gsub("_\\d+$", "", sample_ids)
}
# Replica: il numero finale dopo l'ultimo underscore
...
# ── Pivot long sui parametri numerici ────────────────────────────────────────
pivot_long <- function(df) {
  # Considera parametri: colonne numeriche escluse SampleId, datetime, treatment, replicate
...
ui <- page_fluid(
  ...
  tags$head(tags$style(HTML("
    html, body {
      padding: 1rem 1.5rem !important; margin: 0 !important;
      background: #F5F7F5 !important;
    }
    .wr-card {
      background: #fff; border: 1px solid #DDE5DB; border-radius: 10px;
    ...
    .dl-wrap .btn {
      border: 1.5px solid #3A6B35; color: #3A6B35; background: transparent;
      font-weight: 600; font-size: .95rem; padding: .58rem 1.6rem;
  ...
  tags$script(HTML("
    ...
    function setAll(state) {
      document.querySelectorAll('.col-toggle').forEach(function(el) {
        ...
        el.classList.toggle('excluded', !state);
      });
      updateMeta();
    }
...
server <- function(input, output, session) {
  ...
  output$file_info_chip <- renderUI({
    req(rv$df_raw)
    tx <- t()
    div(class = "file-chip", tx$file_info(nrow(rv$df_raw), ncol(rv$df_raw)))
  })
  ...
  observe({
    ...
    session$sendCustomMessage("updateColMeta",
      list(text = paste(active, "di", ncol(rv$df_raw), t()$col_meta)))
  })
  ...
  observeEvent(input$clean_btn, {
    ...
    tryCatch({
      ...
      df          <- df[, keep_ord, drop = FALSE]
      rv$df_clean <- apply_rename(df)
    }, error = function(e) rv$error <- conditionMessage(e))
  })
  ...
  output$treatment_panel <- renderUI({
    ...
    # Colonna SampleId (dopo rinomina è "SampleId")
    sid_col <- if ("SampleId" %in% colnames(rv$df_clean)) "SampleId"
               else colnames(rv$df_clean)[1]
    samples    <- rv$df_clean[[sid_col]]
    detected   <- extract_treatment(samples)
    replicates <- extract_replicate(samples)

    # Tabella campione → trattamento rilevato → replica → campo personalizzabile
    tbl_rows <- lapply(seq_along(samples), function(i) {
      ...
      tags$tr(
        ...
        tags$td(
          tags$input(type = "text", id = inp_id, class = "trt-input",
            placeholder = det,
    ...
    div(class = "wr-card",
      ...
      div(style = "overflow-x:auto;",
        tags$table(class = "trt-table",
          tags$thead(tags$tr(
            tags$th(tx$trt_col_sample),
            tags$th(tx$trt_col_detected),
            tags$th(tx$trt_col_replica),
            tags$th(tx$trt_col_custom)
          )),
          tags$tbody(tbl_rows)
        )
      ),
  ...
  observeEvent(input$trt_btn, {
    ...
    tryCatch({
      ...
      # Posiziona treatment e replicate subito dopo SampleId
  ...
  output$status_badge <- renderUI({
    ...
    else if (!is.null(rv$df_final)) div(class="status-badge badge-ok",   "✓ Final file ready")
